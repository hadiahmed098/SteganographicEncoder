...
def encode_bits_in_bytes(image_bytes, text_bits):
    ...
    if len(ibytes) < len(tbits) * 8:
        raise IndexError(f"size of image file ({len(ibytes)}) is smaller than size of message ({len(tbits) * 8})")
...
tbits = bytes_to_bits(load_text_bytes('message.txt', size[0] * size[1] * size[2]))

obytes = encode_bits_in_bytes(ibytes, tbits)
save_image_bytes(np.asarray(obytes), 'encoded_message.png', size)
